...
app.layout = html.Div([
                 ...
                 html.Button('Stop', id = 'stop_button', style={'display': 'none'}),
...
@app.callback(Output('start_button','n_clicks'),
             [Input('stop_button','n_clicks')])
def reset_click(n_clicks):
    return 0


@app.callback([Output('start_button', 'style'), Output('stop_button', 'style'), Output('output_start_button', 'children')],
...
#    return 'The input value was "{}" and the button has been clicked {} times'.format(value,n_clicks)
    if value is None or len(value) == 0:
        return {}, {'display': 'none'}, 'Enter your name and press start.'
        
    if n_clicks is not None and n_clicks % 2 != 0:
        return {'display': 'none'}, {}, 'Hi {}. Tempo has started.'.format(value)
    else:
        return {}, {'display': 'none'}, 'Goodbye {}. Tempo has stopped.'.format(value)
...
@app.callback([Output('time_report','children'), Output('detail_report','children')],
               [Input('start_button', 'n_clicks'), Input('time_report_value', 'children'), Input('detail_report_value', 'children')],
               [State('input_box', 'value')])
